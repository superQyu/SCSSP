  ...
  ({ detail, educatedProveUrl }: Props, ref) => {
    ...
    useEffect(() => {
      if (Object.keys(detail).length) {
        ...
        subForm.subcontractorId = ToString(
          subForm.subcontractorId
        );
        subForm.entryStatus = ToString(subForm.entryStatus);
        subForm.valuationMethod = ToString(
          subForm.valuationMethod
        );
        subForm.hasInsurance = ToString(subForm.hasInsurance);
        subForm.laborContractStatus = ToString(
          subForm.laborContractStatus
        );
    ...
    const getSelectOptions = async () => {
      const res1 = await group.getGroupList({ pageSize: '100' });
      // console.log('班组列表', res1);
      const list1 = res1.list.map((item: any) => {
        return { label: item.teamName, value: item.id };
      });
      // console.log('班组列表', list1);
      setGroupList(list1);
    };
