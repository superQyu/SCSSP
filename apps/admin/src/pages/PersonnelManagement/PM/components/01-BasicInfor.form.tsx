...
const DefultForm: React.FC<MenusPropsType> = forwardRef(
  ({ subForm, onFormChange }, ref) => {
    ...
    const init = async () => {
      ...
      if (menus.province) {
        const citys =
          list.filter(
            (item: MenusType) => item.id == menus.province
          )[0] || {};
        setCtyList(citys.children || []);
        if (menus.city) {
          const districts =
            (citys.children || []).filter(
              (item: MenusType) => item.id == menus.city
            )[0] || {};
    ...
    useEffect(() => {
      ...
      const { actualStartTime, actualEndTime, projectImage } =
        subForm[getFormKey] || {};
      setMenus(
    ...
    useEffect(() => {
      init();
      console.log('menus', menus.projectImage);
    }, [menus]);
